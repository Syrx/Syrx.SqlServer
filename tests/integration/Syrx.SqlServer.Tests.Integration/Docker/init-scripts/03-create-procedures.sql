...
BEGIN
    ...
    SELECT @template
        = N'IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''[dbo].[%name]'') AND type in (N''U'')) BEGIN DROP TABLE [dbo].[%name]; END IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''[dbo].[%name]'') AND type in (N''U'')) BEGIN CREATE TABLE [dbo].[%name] ([id] [int] IDENTITY(1, 1) NOT NULL, [name] [varchar](50) NULL, [value] [decimal](18, 2) NULL, [modified] [datetime] NULL, CONSTRAINT [PK_%name] PRIMARY KEY CLUSTERED ([id] ASC)); END;';
...
BEGIN
    SET NOCOUNT ON;
    DECLARE @command nvarchar(max), @template nvarchar(max) = N'BULK INSERT [dbo].[bulk_insert] FROM ''%path'' WITH (FIELDTERMINATOR = '','', ROWTERMINATOR = ''\n'')';
...
BEGIN
    ...
    SELECT @sql = N'TRUNCATE TABLE [dbo].[' + @name + '];';
    EXEC [sys].[sp_executesql] @sql;
END;
GO
